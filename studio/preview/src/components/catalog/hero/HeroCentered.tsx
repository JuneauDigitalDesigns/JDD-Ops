    ...
    <section className="bg-bgSoft px-6 py-20 text-center">
      <div className="mx-auto max-w-3xl">
        ...
        >
          ...
          <p className="mx-auto mt-5 max-w-xl text-lg leading-relaxed text-inkSoft">{hero.sub}</p>
          ...
          >
            {hero.heroBullets.map((b, i) => (
              <div key={b.label} className={`px-5 py-5 ${i % 2 === 0 ? 'bg-bg' : 'bg-bgSoft'}`}>
                <p className="font-heading text-2xl font-bold text-accent">{b.value}</p>
                <p className="mt-1 text-xs text-inkSoft">{b.label}</p>
              </div>
            ))}
          </motion.div>
        )}
