...
/**
 * Project-name and hostname derivation now live in @jdd/schema, and are re-exported here so
 * every existing caller's import keeps working.
 *
 * They moved because they were copied. The underscore→host transform existed inline in four
 * places, three agreed on a rule that was wrong, and the fourth had drifted further by
 * skipping sanitization altogether — and nothing compared them, so nothing noticed. The
 * shared package is unit tested against the real hosts on the live account.
 */
// Imported as well as re-exported: `export … from` creates no local binding, and this
// module calls sanitizeProjectName internally in almost every request path.
import { sanitizeProjectName, vercelAppHost, vercelVoiceUrl } from '@jdd/schema';
export { sanitizeProjectName, vercelAppHost, vercelVoiceUrl };
