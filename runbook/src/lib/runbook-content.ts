    ...
    steps: [
      {
        ...
        blocks: [
          ...
          },
          {
            t: 'callout',
            tone: 'info',
            body: 'Phone numbers are Retell-managed (no Twilio account needed). The provisioned number is saved as TWILIO_NUMBER for the Make scenario’s from_number.',
          },
          {
            t: 'callout',
            tone: 'info',
...
function voicePhase(ctx: ClientContext, config: OpsConfig): Phase {
  return {
    id: 'voice',
    title: 'Voice agent',
    subtitle: 'Tune each agent, then wire post-call logging.',
    steps: [
        ...
        return {
          ...
          blocks: [
            { t: 'copy', label: 'Call this number', value: num.value, pending: num.pending },
            {
              t: 'substeps',
              items: [
                { text: 'Call the number above from your phone. Confirm the greeting names the business correctly.' },
        ...
        blocks: [
          { t: 'cmd', command: 'cp .env.example .env   # fill in, then: npm install', cwd: 'jdd-ops/' },
          {
            ...
            items: [
              ...
              { text: 'RETELL_API_KEY + RETELL_LLM_ID + RETELL_DEFAULT_VOICE_ID — dashboard.retellai.com (agent + numbers).' },
          ...
          { t: 'callout', tone: 'info', body: 'No Twilio account creds (numbers are Retell-managed). Leave TEMPLATE_REPO blank — onboarding is local-first.' },
        ],
      },
    ],
  },
