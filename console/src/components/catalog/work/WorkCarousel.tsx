...
export default function WorkCarousel({ content = CONTENT }: { content?: SiteContent }) {
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          >
            ...
            <p className="mt-2 text-inkSoft"><E p="work.sub">{work.sub}</E></p>
          </motion.div>
                ...
                <div className="relative overflow-hidden" style={{ aspectRatio: '4/3' }}>
                  {p.image?.url ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-bgSoft bg-[repeating-linear-gradient(45deg,transparent,transparent_10px,rgba(0,0,0,0.03)_10px,rgba(0,0,0,0.03)_20px)]">
                      <span className="font-sans text-xs uppercase tracking-widest text-inkSoft"><E p={`work.projects.${i}.caption`}>{p.caption}</E></span>
                    </div>
                  )}
                </div>
                <div className="p-5">
                  ...
                  <div className="mt-1 flex items-start justify-between gap-2">
                    <h3 className="font-heading text-base font-semibold leading-snug text-ink"><E p={`work.projects.${i}.t`}>{p.t}</E></h3>
                    {p.yr && <span className="shrink-0 text-xs text-inkSoft"><E p={`work.projects.${i}.yr`}>{p.yr}</E></span>}
                  </div>
                  <div className="mt-3 space-y-1 text-xs text-inkSoft">
                    <div className="flex gap-2"><span className="font-medium text-ink">Location</span><span><E p={`work.projects.${i}.loc`}>{p.loc}</E></span></div>
                    <div className="flex gap-2"><span className="font-medium text-ink">Size</span><span><E p={`work.projects.${i}.size`}>{p.size}</E></span></div>
                  </div>
                </div>
              </motion.article>
            ))}
