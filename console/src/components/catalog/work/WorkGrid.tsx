...
export default function WorkGrid({ content = CONTENT }: { content?: SiteContent }) {
    ...
    <section id="work" className="bg-bg py-20">
      <div className="mx-auto max-w-6xl px-6">
        ...
        >
          ...
          <p className="mt-3 text-inkSoft"><E p="work.sub">{work.sub}</E></p>
        </motion.div>
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-1 flex items-start justify-between gap-2">
                  <h3 className="font-heading text-sm font-semibold text-ink"><E p={`work.projects.${i}.t`}>{p.t}</E></h3>
                  {p.yr && <span className="shrink-0 text-xs text-inkSoft"><E p={`work.projects.${i}.yr`}>{p.yr}</E></span>}
                </div>
                <p className="mt-1 text-xs text-inkSoft"><E p={`work.projects.${i}.loc`}>{p.loc}</E></p>
              </div>
            </motion.article>
          ))}
        </div>
