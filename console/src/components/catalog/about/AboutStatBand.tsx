...
export default function AboutStatBand({ content = CONTENT }: { content?: SiteContent }) {
    ...
    <section id="about" className="bg-bg py-20">
      <div className="mx-auto max-w-6xl px-6">
        ...
        {about.stats.length > 0 && (
          ...
          >
            {about.stats.map((s, i) => (
              <StatCell key={s.l} n={s.n} l={s.l} np={`about.stats.${i}.n`} lp={`about.stats.${i}.l`} run={bandIn} reduce={reduce} idx={i} />
            ))}
          </div>
        )}
            ...
            {about.pillars.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm font-medium text-ink"><E p={`about.pillars.${i}.t`}>{p.t}</E></span>
                </motion.div>
              );
            })}
          </div>
...
function StatCell({ n, l, np, lp, run, reduce, idx }: { n: string; l: string; np: string; lp: string; run: boolean; reduce: boolean; idx: number }) {
  const val = useCountUp(n, run, reduce, idx * 80);
  return (
    <div className={`px-8 py-10 text-center ${idx % 2 === 0 ? 'bg-ink' : 'bg-ink/90'}`}>
      <p className="font-heading text-4xl font-bold text-bg"><E p={np} fit>{val}</E></p>
