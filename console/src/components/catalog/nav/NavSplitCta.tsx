...
export default function NavSplitCta({ content = CONTENT }: { content?: SiteContent }) {
  ...
  return (
    <header className={`sticky top-0 z-40 bg-bg/95 backdrop-blur transition-all duration-200 ${scrolled ? 'border-b border-rule shadow-sm' : ''}`}>
      <div className={`mx-auto grid max-w-6xl grid-cols-[1fr_auto_1fr] items-center gap-4 px-6 transition-all duration-300 ${scrolled ? 'py-2.5' : 'py-4'}`}>
        {/* Brand (left) */}
        <a href="#top" className={`font-heading font-bold text-ink transition-all duration-300 ${scrolled ? 'text-base' : 'text-lg'}`}><E p="brand.name">{brand.name}</E></a>
            ...
            <nav className="px-6 pb-5 pt-3 space-y-1">
              {nav.map((item, i) => (
                <a key={item.label} href={item.href} onClick={() => setOpen(false)}
                  className="block py-2 text-sm font-medium text-ink"><E p={`nav.${i}.label`}>{item.label}</E></a>
              ))}
