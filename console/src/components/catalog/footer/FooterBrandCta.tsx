...
export default function FooterBrandCta({ content = CONTENT }: { content?: SiteContent }) {
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_1fr_auto]">
          ...
          >
            {logo ? (
              ...
              <p className="mb-4 font-heading text-xl font-bold text-bg"><E p="brand.name">{brand.name}</E></p>
            )}
            <p className="max-w-xs text-sm leading-relaxed text-bg/70"><E p="footer.blurb">{footer.blurb}</E></p>
                ...
                {footer.social.map((s, si) => {
                  ...
                  return (
                    <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                      ...
                      {Icon ? <Icon size={20} /> : <span className="text-xs"><E p={`footer.social.${si}.label`}>{s.label}</E></span>}
                    </a>
                  );
                })}
              </div>
          ...
          <div className="flex flex-col gap-3 lg:min-w-[200px]">
            ...
            {brand.license && <p className="text-xs text-bg/40">Lic. <E p="brand.license">{brand.license}</E></p>}
          </div>
        </div>
