...
export default function FaqTwoColumn({ content = CONTENT }: { content?: SiteContent }) {
            ...
            <div key={ci} className="space-y-5">
              {col.map((f, i) => (
                ...
                >
                  ...
                  <p className="font-semibold text-ink"><E p={`faq.items.${ci * mid + i}.q`}>{f.q}</E></p>
                  <p className="mt-2 leading-relaxed text-inkSoft"><E p={`faq.items.${ci * mid + i}.a`}>{f.a}</E></p>
                </motion.div>
              ))}
            </div>
          ))}
