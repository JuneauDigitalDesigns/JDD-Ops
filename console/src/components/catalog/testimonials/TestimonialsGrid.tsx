...
export default function TestimonialsGrid({ content = CONTENT }: { content?: SiteContent }) {
    ...
    <section id="testimonials" className="bg-bgSoft px-6 py-20">
      <div className="mx-auto max-w-6xl">
        ...
        >
          ...
          <h2 className="mt-2 font-heading text-3xl text-ink md:text-4xl"><E p="testimonials.title">{testimonials.title}</E></h2>
        </motion.div>
            ...
            return (
              ...
              >
                ...
                <div className="mt-6 flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-ink"><E p={`testimonials.items.${i}.a`}>{t.a}</E></p>
                    {(t.r || t.company) && (
                      <p className="text-sm text-inkSoft"><E p={`testimonials.items.${i}.r`}>{t.r}</E>{t.company ? <>, <E p={`testimonials.items.${i}.company`}>{t.company}</E></> : ''}</p>
                    )}
                  </div>
                </div>
              </motion.li>
