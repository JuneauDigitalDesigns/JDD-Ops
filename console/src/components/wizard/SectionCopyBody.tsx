...
/**
 * Per-section copy rendering for the Intake step — an EDITORIAL reader layout (eyebrow /
 * headline / sub / CTA chips / cards), NOT a form, so a reviewer grasps a section at a glance.
 * Neutral cream styling, deliberately not the client's brand skin.
 *
 * Lifted VERBATIM out of SectionCopyModal when copy review moved inline: the modal chrome
 * (backdrop, Esc handling, footer) went away, this did not. Editing still happens in Studio.
 */
...
export default function SectionCopyBody({ section, content }: { section: Section; content: SiteContent }) {
  switch (section) {
    case 'hero': {
